...
- **The document chrome is DocumentGroup's own navigation bar** — the back button, the file name, and its title menu are built by `UIDocumentViewController` from the open document, in the same Pages-style editor layout. `WorkspaceToolbarContent` only adds the trailing search / edit / "more" items, so nothing depends on what the system title menu contains; `.toolbarRole` is left unset, because overriding DocumentGroup's own role has produced a duplicate back button on iPad. The workspace used to hide that bar (`.toolbarVisibility(.hidden, for: .navigationBar)`) and draw a replacement, which is what left a second bar on screen when another app opened a file: bar visibility is navigation-controller state, so DocumentGroup's bar — configured before SwiftUI applies a content preference on that path — survived until the app was relaunched. Do not reintroduce a hand-drawn navigation bar.
